...
const Integrations = () => {
  ...
  const integrationConfigs = {
    github: {
      ...
      icon: Github,
      color: 'bg-gray-900',
      description: 'Sincronize tarefas como issues do GitHub',
    ...
    trello: {
      ...
      icon: Trello,
      color: 'bg-blue-600',
      description: 'Converta tarefas em cards do Trello',
      docs: 'https://developer.atlassian.com/cloud/trello/rest/',
      fields: [
        { key: 'trello_key', label: 'API Key', type: 'password', required: true },
    ...
    notion: {
      ...
      icon: Database,
      color: 'bg-black',
      description: 'Crie páginas no Notion a partir de tarefas',
    ...
    capacities: {
      ...
      icon: BookOpen,
      color: 'bg-purple-600',
      description: 'Sincronize com seu knowledge graph',
      docs: 'https://docs.capacities.io/developer/api',
      fields: [
        { key: 'capacities_token', label: 'API Token', type: 'password', required: true }
      ]
    },
    obsidian: {
      name: 'Obsidian',
      icon: FileText,
      color: 'bg-indigo-600',
      description: 'Exporte/importe notas para vault local',
  ...
  const loadSavedCredentials = () => {
    const saved = localStorage.getItem('lex-flow-credentials');
    if (saved) {
      try {
  ...
  const saveCredentials = () => {
    localStorage.setItem('lex-flow-credentials', JSON.stringify(credentials));
  ...
  const syncTasks = async () => {
    ...
    if (activeTasks.length === 0) {
      alert('Nenhuma tarefa ativa para sincronizar');
      return;
    }
    ...
    try {
      ...
      if (syncTargets.github_repo) targets.github = syncTargets.github_repo;
      if (syncTargets.trello_list) targets.trello = syncTargets.trello_list;
      if (syncTargets.notion_database) targets.notion = syncTargets.notion_database;

      const response = await fetch('/api/integrations/sync/tasks', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        alert(`${data.synced_tasks} tarefas sincronizadas com sucesso!`);
      }
    } catch (error) {
  ...
  const exportToObsidian = async () => {
    if (!syncTargets.obsidian_vault) {
      alert('Por favor, especifique o caminho do vault do Obsidian');
      return;
    }
    ...
    try {
      const response = await fetch('/api/integrations/obsidian/export', {
        ...
        body: JSON.stringify({
          vault_path: syncTargets.obsidian_vault,
          notes: notes
        })
      });
      ...
      if (data.success) {
        alert(`${data.exported_count} notas exportadas para o Obsidian!`);
      } else {
        alert('Erro ao exportar para Obsidian: ' + data.error);
      }
    } catch (error) {
      console.error('Erro ao exportar para Obsidian:', error);
      alert('Erro ao exportar para Obsidian');
    } finally {
      setLoading(false);
    }
  };
          ...
          return (
            <div key={key} className="p-4 border border-gray-200 rounded-lg hover:shadow-md transition-shadow">
              ...
              <div className="space-y-2 text-sm">
                ...
                {connectionStatus && (
                  <>
                    {connectionStatus.repos_count !== undefined && (
                      <div className="flex justify-between">
                        <span className="text-gray-600">Repositórios:</span>
                        <span className="font-medium">{connectionStatus.repos_count}</span>
                      </div>
                    )}
                    {connectionStatus.boards_count !== undefined && (
                      <div className="flex justify-between">
                        <span className="text-gray-600">Boards:</span>
                        <span className="font-medium">{connectionStatus.boards_count}</span>
                      </div>
                    )}
                    {connectionStatus.databases_count !== undefined && (
                      <div className="flex justify-between">
                        <span className="text-gray-600">Databases:</span>
                        <span className="font-medium">{connectionStatus.databases_count}</span>
                      </div>
                    )}
                    {connectionStatus.objects_count !== undefined && (
                      <div className="flex justify-between">
                        <span className="text-gray-600">Objetos:</span>
                        <span className="font-medium">{connectionStatus.objects_count}</span>
                      </div>
                    )}
      ...
      <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
        <div className="flex items-start gap-2">
          ...
          <div className="text-sm text-yellow-800">
            ...
            <p>As credenciais são armazenadas localmente no seu navegador e nunca são enviadas para servidores externos. Para máxima segurança, use tokens com permissões mínimas necessárias.</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              value={syncTargets.github_repo}
              onChange={(e) => setSyncTargets(prev => ({ ...prev, github_repo: e.target.value }))}
          ...
          <div>
            ...
            <input
              type="text"
              value={syncTargets.trello_list}
              onChange={(e) => setSyncTargets(prev => ({ ...prev, trello_list: e.target.value }))}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              placeholder="ID da lista do Trello"
            />
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              value={syncTargets.notion_database}
              onChange={(e) => setSyncTargets(prev => ({ ...prev, notion_database: e.target.value }))}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          ...
          <div>
            ...
            <input
              type="text"
              value={syncTargets.obsidian_vault}
              onChange={(e) => setSyncTargets(prev => ({ ...prev, obsidian_vault: e.target.value }))}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              placeholder="/caminho/para/vault"
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 disabled:opacity-50"
            >
          ...
          {syncResults && (
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              <h5 className="font-medium text-green-800 mb-2">Resultado da Sincronização</h5>
              <div className="text-sm text-green-700 space-y-1">
                <p>Tarefas sincronizadas: {syncResults.synced_tasks}</p>
                {syncResults.results.github.length > 0 && (
                  <p>GitHub: {syncResults.results.github.length} issues criadas</p>
                )}
                {syncResults.results.trello.length > 0 && (
                  <p>Trello: {syncResults.results.trello.length} cards criados</p>
                )}
                {syncResults.results.notion.length > 0 && (
                  <p>Notion: {syncResults.results.notion.length} páginas criadas</p>
                )}
                {syncResults.results.errors.length > 0 && (
                  <div className="text-red-600">
                    <p>Erros: {syncResults.results.errors.length}</p>
      ...
      <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <div className="flex items-start gap-2">
          ...
          <div className="text-sm text-blue-800">
            <p className="font-medium mb-1">Como funciona a sincronização</p>
            <ul className="space-y-1 list-disc list-inside">
              <li>Tarefas ativas são convertidas em issues/cards/páginas</li>
              <li>Metadados como prioridade e categoria são preservados</li>
              <li>Labels automáticos são adicionados baseados no contexto</li>
              <li>A sincronização não afeta as tarefas originais no Lex Flow</li>
            </ul>
          </div>
        </div>
      </div>
    ...
    <div className="max-w-6xl mx-auto p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-2">
          Conecte o Lex Flow com suas ferramentas favoritas de produtividade
        </p>
      </div>
