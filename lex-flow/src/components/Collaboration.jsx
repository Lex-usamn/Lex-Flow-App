...
const Collaboration = () => {
  ...
  const initializeSocket = () => {
    ...
    newSocket.on('task_updated', (data) => {
      ...
      // Atualizar interface com mudanças em tempo real
    });
  ...
  const loadProjectDetails = async (projectId) => {
    try {
      ...
      const collabResponse = await fetch(`/api/collaboration/projects/${projectId}/collaborators`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      // Carregar comentários (se houver tarefas)
  ...
  const inviteCollaborator = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const response = await fetch(`/api/collaboration/projects/${selectedProject.id}/invite`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const addComment = async () => {
    ...
    try {
      ...
      // Simular adição de comentário (precisaria de uma tarefa específica)
      const comment = {
        id: Date.now(),
        content: newComment,
        user_id: user.id,
      ...
      // Emitir via socket para outros usuários
      if (socket) {
